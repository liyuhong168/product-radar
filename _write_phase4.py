#!/usr/bin/env python3
"""Helper to write Phase 4 scripts. Run once, then delete."""
from pathlib import Path
import json

BASE = Path("/home/lee/product-radar")

# ============================================================
# 1. competitor_monitor.py
# ============================================================
(BASE / "competitor_monitor.py").write_text(r'''#!/usr/bin/env python3
"""
...
_API_KEY_FILE = Path(os.path.expanduser("~")) / ".hermes" / "scraperapi_key.txt"
...
def _parse_price(html):
    for pat in [
        ...
        r'"priceAmount":\s*([\d.]+)',
        r'<span[^>]*id="priceblock_ourprice"[^>]*>\xa3([\d.]+)</span>',
    ]:
...
def _detect_alerts(asin, current, history):
    ...
    if not history:
        alerts.append({
            "asin": asin, "type": "new_tracking", "severity": "info",
    ...
    if current.get("price") and prev.get("price"):
        ...
        if drop_pct > 10:
            ...
            alerts.append({
                "asin": asin, "type": "price_drop", "severity": severity,
                "message": f"Price dropped {drop_pct:.0f}%: \u00a3{prev['price']:.2f} -> \u00a3{current['price']:.2f}",
                "old": prev["price"], "new": current["price"],
                "change_pct": round(drop_pct, 1),
            })

    # Review spike >50 in 7 days
    if current.get("reviews") is not None:
        ...
        if week_old and week_old[-1].get("reviews") is not None:
            ...
            if review_delta > 50:
                alerts.append({
                    "asin": asin, "type": "review_spike", "severity": "warning",
                    "message": f"Review count jumped +{review_delta} in 7 days ({week_old[-1]['reviews']} -> {current['reviews']})",
                    "old": week_old[-1]["reviews"], "new": current["reviews"],
                })

    # BSR drop >20% (higher number = worse rank)
    if current.get("bsr") and prev.get("bsr") and prev["bsr"] > 0:
        bsr_change_pct = (current["bsr"] - prev["bsr"]) / prev["bsr"] * 100
        if bsr_change_pct > 20:
            alerts.append({
                "asin": asin, "type": "bsr_drop", "severity": "warning",
                "message": f"BSR worsened {bsr_change_pct:.0f}%: #{prev['bsr']:,} -> #{current['bsr']:,}",
                "old": prev["bsr"], "new": current["bsr"],
                "change_pct": round(bsr_change_pct, 1),
            })
        elif bsr_change_pct < -20:
            alerts.append({
                "asin": asin, "type": "bsr_improvement", "severity": "info",
                "message": f"BSR improved {abs(bsr_change_pct):.0f}%: #{prev['bsr']:,} -> #{current['bsr']:,}",
                "old": prev["bsr"], "new": current["bsr"],
                "change_pct": round(bsr_change_pct, 1),
            })
    ...
    if current.get("rating") and prev.get("rating"):
        rating_delta = current["rating"] - prev["rating"]
        if abs(rating_delta) >= 0.3:
            alerts.append({
                "asin": asin, "type": "rating_change",
                "severity": "warning" if rating_delta < 0 else "info",
    ...
    if not alerts:
        alerts.append({
            "asin": asin, "type": "stable", "severity": "info",
...
def monitor_competitors(asin_list: list[str]) -> list[dict]:
    """
    Monitor competitors for a list of ASINs.
    Returns list of alerts with severity (info/warning/critical).
    """
    api_key = _load_api_key()
    if not api_key:
        print("[ERROR] No ScraperAPI key found.", file=sys.stderr)
        return [{"asin": a, "type": "error", "severity": "critical",
                 "message": "No API key"} for a in asin_list]
    ...
    for asin in asin_list:
        ...
        if "error" in data:
            all_alerts.append({
                "asin": asin, "type": "error", "severity": "critical",
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        sys.exit(1)
    asins = sys.argv[1:]
    results = monitor_competitors(asins)
    print(json.dumps(results, indent=2, ensure_ascii=False))
''')

print("competitor_monitor.py:", len((BASE / "competitor_monitor.py").read_text()), "bytes")
